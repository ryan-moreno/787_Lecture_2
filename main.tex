...
\begin{itemize}
    ...
    \item \textbf{Example of a UHF}: $\mathrm{H} = \pmb{ \{ }h_{ab} | a \in \{1, ... , p-1\}, b \in \{0, ..., p-1\}  \pmb{ \} }$ s.t.
...
\begin{itemize}
    \item 
    Note that by Markov's inequality, $\pr \pmb{[}\expc\left[\sum_{i=1}^{n}s_i^2\right] \geq 4n\pmb{]} \leq \frac{2n-1}{4n} \leq \frac{1}{2}$. So, we should only have to try a small number of hash functions in order to get a total second-level size < $4n$.
    ...
    \item
    Note also that this solution is for a static data structure (meaning that we know $S$ when we create the two-level hash table). We must know $S$ because we need to know the value $s_i$ (how many elements map to position $i$ in the first-level array) for each value of $i$.
    
    \item
    Note that we also need $\log{|U|}$ bits for each position in the second-level arrays because we need to store the value $x \in S$ that maps to a given position. We need to store the values (rather than merely storing a boolean value) because we only prevented collisions between two values in $S$. If you hash a value $y$ to position $i$ where $y \in U$ and $y \notin S$, it could collide with a value $x \in S$ also hashed to $i$. In this case, when we hash $y$ it would be imperative to compare the value $y$ to the value stored in position $i$.
\end{itemize}
...
$\pr \left \pmb{[} |x - \expc[x]| \geq k \sigma \right \pmb{]} \leq \frac{1}{k^2}$ if $x$ is a random variable with $\var[x] = \sigma^2$.
...
Let $\mu = \expc[x]$. We begin with the left-hand side of Chebyshev's Inequality
 ...
 \[ \pr \left \pmb{[} |x - \mu| \geq k \sigma \right \pmb{]} = \pr \left \pmb{[} (x - \mu)^2 \geq (k \sigma)^2 \right \pmb{]} \]
...
As an aside, if the hashing function was perfectly random, then the max load would be $\bigo(\log n)$ instead of \bigo($\sqrt{n})$.

\section{Chernoff Bound}
\subsubsection{Statement of Chernoff Bound}
Let $x = \sum_{i=0}^{n} x_i$ where $x_i$ are independent random variables that take values in $\{0, 1\}$ and $\exp[x] = \mu$. Then $\pr[x \geq (1+\varepsilon)\mu] \leq \left(\frac{e^\varepsilon}{(1+\varepsilon)^{1+\varepsilon}}\right)^\mu \leq e^{-\frac{\varepsilon^2 \mu}{2+\varepsilon}}$
...
If we choose $k= \log n$ then with good probability no bin will have $\log n$ balls in it. (TODO: why??)
